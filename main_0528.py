...
from data_0528 import NYUDataset, rgb_data_transforms, depth_data_transforms
# from data import NYUDataset, NYUDataset_train, rgb_data_transforms, depth_data_transforms, depth_data_transforms_coarse, depth_data_transforms_fine
...
from model_0528_new import coarseNet, fineNet
# from model_0528_pretrn import coarseNet, fineNet
...
r = 1
coarse_optimizer = torch.optim.SGD([{'params': coarse_model.fc1.parameters(), 'lr': 0.1*r},
                                    {'params': coarse_model.fc2.parameters(), 'lr': 0.1*r}], 
                                   lr=0.001*r, momentum=0.9, weight_decay=0.01)
...
fine_optimizer = optim.SGD([{'params': fine_model.conv1.parameters(), 'lr': 0.001},{'params': fine_model.conv2.parameters(), 'lr': 0.01},{'params': fine_model.conv3.parameters(), 'lr': 0.001}], lr = 0.001, momentum = 0.9, weight_decay=0)
...
# coarse_optimizer = optim.Adam(coarse_model.parameters(), lr=0.0005, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
...
def custom_loss_function(output, target):
    # di = output - target
    di = target - output #shape: batch_size*1*55*74
    n = (output_height * output_width) #55*74  #n pixels
    di2 = torch.pow(di, 2)
    first_term = torch.sum(di2,(1,2,3))/n #shape: batch_size
    second_term = 0.5*torch.pow(torch.sum(di,(1,2,3)), 2)/ (n**2) #shape: batch_size
...
#     a = a.cuda()
#     b = b.cuda()   
#     G_x = F.conv2d(d_batch, a, padding=1).cuda()
#     G_y = F.conv2d(d_batch, b, padding=1).cuda() 
...
# def custom_loss_function(output, target):
#     di = target - output #shape: batch_size*1*55*74
# #     di = torch.log(preds) - torch.log(actual_depth)

#     n = (output_height * output_width) #55*74  #n pixels
#     di2 = torch.pow(di, 2)
#     first_term = torch.sum(di2,(1,2,3))/n #shape: batch_size
#     second_term = 0.5*torch.pow(torch.sum(di,(1,2,3)), 2)/ (n**2) #shape: batch_size
#     third_term = im_gradient_loss(di, n)
...
#         print("coarse output", output)
#         # log????
#         output = torch.log(output)
#         print("coarse log output", output)
